"""Frontend compatibility patch for notification snooze semantics.

The existing app.js predates the current scheduling API schema. This patch is
appended to /static/app.js at response time so the source file can remain
untouched while the UI exposes notification snooze and motion detection as
separate controls.
"""
...
// MyBlink notification-snooze semantic fixes.
(() => {
    ...
    function transformScheduleCreatePayload(payload) {
        ...
        switch (selectedAction) {
            ...
            case 'sync_disarm':
                ...
                break;
            default:
                // Also translate the old rule_type/action shape if this request
                // was created before the DOM option patch ran.
    ...
    window.fetch = async function(input, init = {}) {
        ...
        if (url === '/api/schedules' && method === 'POST' && init.body) {
            try {
                ...
                const transformed = transformScheduleCreatePayload(originalPayload);
                effectiveInit = {
                    ...init,
        ...
        root.querySelectorAll('.setting-label').forEach(element => {
            ...
            if (text === 'Snooze Motion') {
                element.childNodes.forEach(node => {
                    if (node.nodeType === Node.TEXT_NODE && node.textContent.includes('Snooze Motion')) {
                        node.textContent = node.textContent.replace('Snooze Motion', 'Snooze Notifications');
                    }
                });
            } else if (text === 'Arm Camera') {
                element.childNodes.forEach(node => {
                    if (node.nodeType === Node.TEXT_NODE && node.textContent.includes('Arm Camera')) {
                        node.textContent = node.textContent.replace('Arm Camera', 'Motion Detection');
                    }
                });
            }
        });
    ...
    const observer = new MutationObserver(() => patchCameraLabels(document));
    observer.observe(document.documentElement, { childList: true, subtree: true });
    document.addEventListener('DOMContentLoaded', () => patchCameraLabels(document));
})();
"""
...
def apply_notification_snooze_ui() -> None:
    """Append the notification-snooze frontend patch to app.js responses."""
    ...
    def patched_init(self, *args, **kwargs):
        ...
        def patched_app_js(*view_args, **view_kwargs):
            ...
            if "MyBlink notification-snooze semantic fixes" not in javascript:
